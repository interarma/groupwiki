...
EOT
	...
	protected function __recordsCount( $customQuery = null )
	{
		...
		{
			// PRD: commented out to enable non-trivial nested SELECTs
			//$query = preg_replace( '§^SELECT .+ (FROM .+)$§i',
			//					   'SELECT COUNT(*) \1', $customQuery );
			$query = 'SELECT COUNT(*) FROM ( ' . $customQuery . ' )';
...
EOT;
	...
	protected function getGlobalCommands()
	{
		...
		// PRD: commented out 'drop' to prevent 'casual' wiki admins (i.e. those who've just installed
		//      a pre-configured wiki as an application and aren't familiar enough to know how to use
		//      the console to drop the table manually) from nuking their databases
		$globalCmds = array(
		...
		switch( $def['format'] )
		{
			...
			case 'bool' :
				if ( !$value && $mayBeSkipped )
					return null;
				// PRD: changed from a 'X' to a tick
				//return $value ? '&#10007;' : '&ndash;';
				return $value ? '&#10003;' : '&ndash;';
